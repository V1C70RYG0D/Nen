...
class MockAuthenticationService {
    ...
    generateTokens(user) {
        ...
        const refreshToken = jwt.sign({ userId: user.id, type: 'refresh' }, this.refreshSecret, { expiresIn: '7d' });
        return {
            token,
            refreshToken,
            user: { ...user },
...
describe('User Authentication & Authorization', () => {
    ...
    describe('JWT Token Generation and Validation', () => {
        ...
        test('should generate refresh tokens with extended expiration', async () => {
            ...
            const decoded = jwt.verify(result.refreshToken, 'test-refresh-secret');
            expect(decoded.userId).toBe(result.user.id);
            expect(decoded.type).toBe('refresh');
            expect(decoded.exp).toBeGreaterThan(Date.now() / 1000 + 6 * 24 * 60 * 60);
        });
        ...
        test('should reject expired JWT tokens', async () => {
            const expiredToken = jwt.sign({ userId: 'test-user', wallet: VALID_WALLET }, 'test-jwt-secret', { expiresIn: '-1h' });
    ...
    describe('Token Expiration Handling', () => {
        ...
        test('should handle token cleanup on expiration', async () => {
            const shortLivedToken = jwt.sign({ userId: 'test-user', wallet: VALID_WALLET }, 'test-jwt-secret', { expiresIn: '1ms' });
        ...
        test('should validate token expiration in different timezones', async () => {
            ...
            Date.now = () => originalNow() + 23 * 60 * 60 * 1000;
            try {
                const user = await authService.verifyToken(result.token);
                expect(user).toBeDefined();
            }
    ...
    describe('Refresh Token Mechanism', () => {
        ...
        test('should reject expired refresh tokens', async () => {
            const expiredRefreshToken = jwt.sign({ userId: 'test-user', type: 'refresh' }, 'test-refresh-secret', { expiresIn: '-1h' });
    ...
    describe('Security Edge Cases and Error Handling', () => {
        ...
        test('should handle concurrent registration attempts', async () => {
            ...
            const results = await Promise.allSettled(registrationRequests.map(req => authService.register(req)));
    ...
    describe('JWT Token Generation and Validation', () => {
        ...
        test('should generate refresh tokens with extended expiration', async () => {
            ...
            const result = await authService.register(registrationRequest);
            expect(result.refreshToken).toMatch(/^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/);
            const decoded = jwt.verify(result.refreshToken, REFRESH_SECRET);
            expect(decoded.userId).toBe(result.user.id);
            expect(decoded.type).toBe('refresh');
            expect(decoded.exp).toBeGreaterThan(Date.now() / 1000 + 6 * 24 * 60 * 60);
        });
        ...
        test('should reject expired JWT tokens', async () => {
            const expiredToken = jwt.sign({ userId: 'test-user', wallet: VALID_WALLET }, JWT_SECRET, { expiresIn: '-1h' });
            const user = await authService.verifyToken(expiredToken);
            expect(user).toBeNull();
        });
        test('should handle tokens for deactivated users', async () => {
    ...
    describe('Token Expiration Handling', () => {
        ...
        test('should handle token cleanup on expiration', async () => {
            const shortLivedToken = jwt.sign({ userId: 'test-user', wallet: VALID_WALLET }, JWT_SECRET, { expiresIn: '1ms' });
            await new Promise(resolve => setTimeout(resolve, 10));
            const user = await authService.verifyToken(shortLivedToken);
            expect(user).toBeNull();
        });
    ...
    describe('Refresh Token Mechanism', () => {
        ...
        test('should reject expired refresh tokens', async () => {
            const expiredRefreshToken = jwt.sign({ userId: 'test-user', type: 'refresh' }, REFRESH_SECRET, { expiresIn: '-1h' });
            await expect(authService.refreshToken(expiredRefreshToken)).rejects.toThrow('Refresh token expired');
        });
        test('should invalidate old tokens when refreshing', async () => {
            const registrationRequest = {
