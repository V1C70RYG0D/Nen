...
describe('Odds Calculation & Pool Management', () => {
    ...
    test('Performance with large bet volumes', async () => {
        ...
        for (let i = 0; i < numberOfBets; i++) {
            ...
            const amount = (Math.random() * 5 + 0.1) * web3_js_1.LAMPORTS_PER_SOL;
    ...
    test('Pool synchronization across services', async () => {
        ...
        const optimizedResult1 = await optimizedBettingService.placeBet(TEST_BETTOR_1, optimizedMatchId, 2.0 * web3_js_1.LAMPORTS_PER_SOL, AGENT_ID_1, 'ai_agent');
        const optimizedResult2 = await optimizedBettingService.placeBet(TEST_BETTOR_2, optimizedMatchId, 3.0 * web3_js_1.LAMPORTS_PER_SOL, AGENT_ID_2, 'ai_agent');
        expect(optimizedResult1.success).toBe(true);
        expect(optimizedResult2.success).toBe(true);
        expect(finalEnhancedPool).not.toBeNull();
...
//# sourceMappingURL=odds-calculation.test.js.map