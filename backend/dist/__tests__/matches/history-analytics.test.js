...
class MockMatchHistoryService {
    ...
    generateMockData() {
        ...
        for (let i = 0; i < 100; i++) {
            ...
            const createdAt = (0, date_fns_1.subDays)(now, Math.floor(Math.random() * 30));
            const duration = 300 + Math.floor(Math.random() * 1200);
            const totalMoves = 15 + Math.floor(Math.random() * 85);
            const match = {
                id: (0, uuid_1.v4)(),
                matchId,
    ...
    generateMockMoveHistory(totalMoves) {
        const moves = [];
    ...
    getMatchesByPlayer(playerId) {
        return Array.from(this.matchHistory.values())
            .filter(match => match.player1Id === playerId ||
            match.player2Id === playerId ||
            match.aiAgent1Id === playerId ||
            match.aiAgent2Id === playerId);
    }
    async calculatePerformanceAnalytics(options = {}) {
        ...
        const winRatesByAgent = new Map();
        const agentGames = new Map();
    ...
    async archiveOldMatches(config) {
        const cutoffDate = (0, date_fns_1.subDays)(new Date(), config.retentionPeriodDays);
        let archivedCount = 0;
...
describe('Match History & Analytics', () => {
    ...
    describe('Match data storage and retrieval', () => {
        ...
        test('should retrieve matches by date range with proper filtering', async () => {
            const now = new Date();
            const startDate = (0, date_fns_1.subDays)(now, 7);
            const endDate = now;
    ...
    describe('Performance analytics calculation', () => {
        ...
        test('should calculate agent performance metrics accurately', async () => {
            ...
            if (agentMatches.length > 0) {
                const wins = agentMatches.filter(match => (match.aiAgent1Id === targetAgent && match.winnerId === 'agent1') ||
                    (match.aiAgent2Id === targetAgent && match.winnerId === 'agent2')).length;
        ...
        test('should calculate time-based performance patterns', async () => {
            const now = new Date();
            const startDate = (0, date_fns_1.subDays)(now, 30);
        ...
        test('should calculate match duration distributions and patterns', async () => {
            const analytics = await mockHistoryService.calculatePerformanceAnalytics();
            const matches = mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(new Date(), 30), new Date());
            ...
            expect(analytics.averageMatchDuration).toBeCloseTo(durations.reduce((sum, d) => sum + d, 0) / durations.length, 1);
        });
    });
    describe('Statistical aggregation', () => {
        test('should aggregate daily match statistics', async () => {
            ...
            matches.forEach(match => {
                const dateKey = (0, date_fns_1.format)(match.createdAt, 'yyyy-MM-dd');
                ...
                if (match.player1Id)
                    stats.uniquePlayers.add(match.player1Id);
                if (match.player2Id)
                    stats.uniquePlayers.add(match.player2Id);
                if (match.aiAgent1Id)
                    stats.uniquePlayers.add(match.aiAgent1Id);
                if (match.aiAgent2Id)
                    stats.uniquePlayers.add(match.aiAgent2Id);
            });
            ...
            dailyStats.forEach((stats, date) => {
                ...
                expect((0, date_fns_1.parseISO)(date)).toBeInstanceOf(Date);
            });
        });
        ...
        test('should aggregate agent performance across multiple metrics', async () => {
            ...
            matches.forEach(match => {
                const agents = [match.aiAgent1Id, match.aiAgent2Id].filter(Boolean);
            ...
            agentPerformance.forEach((stats, agentId) => {
                const agentMatches = matches.filter(m => m.aiAgent1Id === agentId || m.aiAgent2Id === agentId);
    ...
    describe('Trend analysis functionality', () => {
        test('should detect performance trends over time', async () => {
            const now = new Date();
            const matches = mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(now, 30), now);
            const weeklyData = [];
            const weeks = 4;
            for (let i = 0; i < weeks; i++) {
                const weekStart = (0, date_fns_1.subDays)(now, (i + 1) * 7);
                const weekEnd = (0, date_fns_1.subDays)(now, i * 7);
                const weekMatches = matches.filter(m => m.createdAt >= weekStart && m.createdAt < weekEnd);
                if (weekMatches.length > 0) {
                    ...
                    weekMatches.forEach(match => {
                        ...
                        if (match.aiAgent1Id)
                            uniquePlayers.add(match.aiAgent1Id);
                        if (match.aiAgent2Id)
                            uniquePlayers.add(match.aiAgent2Id);
                    });
                    weeklyData.push({
        ...
        test('should identify popular time patterns and peak usage', async () => {
            const matches = mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(new Date(), 7), new Date());
    ...
    describe('Data visualization preparation', () => {
        test('should prepare time series data for charts', async () => {
            ...
            const chartData = Array.from(timeSeriesData.values())
                ...
                .map(data => {
                const dayMatches = matches.filter(m => (0, date_fns_1.format)(m.createdAt, 'yyyy-MM-dd') === data.date);
                data.averageDuration = dayMatches.reduce((sum, m) => sum + m.duration, 0) / dayMatches.length;
                const uniqueAgents = new Set();
                dayMatches.forEach(match => {
                    if (match.aiAgent1Id)
                        uniqueAgents.add(match.aiAgent1Id);
                    if (match.aiAgent2Id)
                        uniqueAgents.add(match.aiAgent2Id);
                });
    ...
    describe('Historical data query optimization', () => {
        test('should optimize database queries for large datasets', async () => {
            ...
            expect(optimizationResult.queryTime).toBeLessThan(100);
        });
        test('should implement efficient pagination for match history', async () => {
            ...
            for (let page = 0; page < Math.ceil(totalMatches / pageSize); page++) {
                ...
                const matches = await mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(new Date(), 30), new Date());
        ...
        test('should cache frequently accessed analytics data', async () => {
            ...
            expect(analytics1.totalMatches).toBe(analytics2.totalMatches);
        ...
        test('should optimize queries for specific date ranges', async () => {
            const testCases = [
                { days: 1, expectedTime: 20 },
                { days: 7, expectedTime: 30 },
                { days: 30, expectedTime: 50 },
                { days: 90, expectedTime: 100 }
            ];
            for (const testCase of testCases) {
                const startTime = performance.now();
                const matches = await mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(new Date(), testCase.days), new Date());
                const queryTime = performance.now() - startTime;
    ...
    describe('Archive management', () => {
        ...
        test('should implement progressive archive compression', async () => {
            ...
            expect(storageStats.compressionRatio).toBeGreaterThan(0.3);
            expect(storageStats.compressionRatio).toBeLessThan(1);
    ...
    describe('Export functionality', () => {
        test('should export match data in multiple formats', async () => {
            const exportOptions = {
                format: 'json',
                dateRange: {
                    start: (0, date_fns_1.subDays)(new Date(), 7),
                    end: new Date()
                },
        ...
        test('should provide real-time analytics with WebSocket support', async () => {
            const realtimeUpdate = {
                ...
                data: {
                    ...
                    recentMatches: await mockHistoryService.getMatchesByDateRange((0, date_fns_1.subDays)(new Date(), 1), new Date())
                }
            };
            ...
            const dashboardData = {
                ...
                data: {
                    ...
                    volumeChart: Array.from({ length: 7 }, (_, i) => ({
                        date: (0, date_fns_1.format)((0, date_fns_1.subDays)(new Date(), 6 - i), 'yyyy-MM-dd'),
...
//# sourceMappingURL=history-analytics.test.js.map