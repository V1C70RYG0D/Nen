...
class MockTransactionProposalService {
    ...
    async executeTransaction(proposalId, executor, options) {
        ...
        const vaultObj = this.vaultService.vaults.get(proposal.vaultId);
        if (vaultObj) {
...
(0, globals_1.describe)('Multi-Signature Vault Security - Optimized', () => {
    let mockConnection;
    let mockRedisClient;
    let multisigService;
    let proposalService;
    ...
    (0, globals_1.beforeAll)(async () => {
        mockConnection = {
            getBalance: globals_1.jest.fn().mockResolvedValue(10 * web3_js_1.LAMPORTS_PER_SOL),
            requestAirdrop: globals_1.jest.fn().mockResolvedValue('mock_airdrop_signature'),
            confirmTransaction: globals_1.jest.fn().mockResolvedValue({ value: { err: null } }),
            sendTransaction: globals_1.jest.fn().mockResolvedValue('mock_transaction_signature')
        };
        mockRedisClient = {
            hgetall: globals_1.jest.fn().mockResolvedValue({}),
            hset: globals_1.jest.fn().mockResolvedValue('OK'),
        ...
        testVaults = [];
        logger_1.logger.info('🚀 Mock multi-signature vault test environment initialized');
    }, 10000);
    (0, globals_1.afterAll)(async () => {
        await mockRedisClient.quit();
        logger_1.logger.info('✅ Mock test environment cleaned up');
    });
    ...
    (0, globals_1.test)('3-of-5 operational vault creation', async () => {
        ...
        (0, globals_1.expect)(mockRedisClient.hset).toHaveBeenCalledWith(`vault:${vault.id}`, globals_1.expect.objectContaining({
            id: vault.id,
            requiredSignatures: '3',
            totalSigners: '5',
            type: 'operational'
    ...
    (0, globals_1.test)('Transaction proposal mechanism', async () => {
        ...
        const latency = performance.now() - startTime;
        (0, globals_1.expect)(latency).toBeLessThan(500);
    ...
    (0, globals_1.test)('Signature collection workflow', async () => {
        ...
        await (0, globals_1.expect)(proposalService.addSignature(proposal.id, operationalSigners[1])).rejects.toThrow('already signed');
        await (0, globals_1.expect)(proposalService.addSignature(proposal.id, unauthorizedUser)).rejects.toThrow('not authorized');
        ...
        await (0, globals_1.expect)(proposalService.addSignature(expiredProposal.id, operationalSigners[4])).rejects.toThrow('expired');
        const latency = performance.now() - startTime;
        (0, globals_1.expect)(latency).toBeLessThan(500);
    ...
    (0, globals_1.test)('Transaction execution with required signatures', async () => {
        ...
        await (0, globals_1.expect)(proposalService.executeTransaction(insufficientProposal.id, operationalSigners[3])).rejects.toThrow('insufficient signatures');
        await (0, globals_1.expect)(proposalService.executeTransaction(proposal.id, operationalSigners[0])).rejects.toThrow('already executed');
        const latency = performance.now() - startTime;
        (0, globals_1.expect)(latency).toBeLessThan(500);
    ...
    (0, globals_1.test)('Vault balance tracking', async () => {
        ...
        (0, globals_1.expect)(balance3).toBe(fundingAmount - (2 * web3_js_1.LAMPORTS_PER_SOL));
        const latency = performance.now() - startTime;
        (0, globals_1.expect)(latency).toBeLessThan(500);
    ...
    (0, globals_1.test)('Vault access control', async () => {
        ...
        const unauthorizedAccess = await multisigService.checkSignerAccess(vault.id, unauthorizedUser.publicKey);
        (0, globals_1.expect)(unauthorizedAccess).toBe(false);
        await (0, globals_1.expect)(proposalService.createProposal({
            vaultId: vault.id,
            recipient: web3_js_1.Keypair.generate().publicKey,
            amount: 1 * web3_js_1.LAMPORTS_PER_SOL,
    ...
    (0, globals_1.test)('Transaction history and auditing', async () => {
        ...
        (0, globals_1.expect)(history.length).toBeGreaterThanOrEqual(4);
        const latency = performance.now() - startTime;
        (0, globals_1.expect)(latency).toBeLessThan(500);
...
//# sourceMappingURL=multisig-vaults-optimized.test.js.map