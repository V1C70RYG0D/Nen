...
describe('Rate Limiting & Security', () => {
    ...
    describe('Rate limiter blocks excessive requests (>100/15min)', () => {
        test('should allow requests up to limit', async () => {
            const requests = Array.from({ length: 50 }, (_, i) => (0, supertest_1.default)(testApp.app)
                .get('/api/test')
                .expect(200));
        ...
        test('should include correct rate limit information', async () => {
            const response = await (0, supertest_1.default)(testApp.app)
                .get('/api/test')
                .expect(200);
            expect(response.headers['x-ratelimit-limit']).toBe('100');
    ...
    describe('IP-based limiting works correctly', () => {
        test('should track different IPs separately', async () => {
            const ip1Response = await (0, supertest_1.default)(testApp.app)
                .get('/api/test')
                .set('X-Forwarded-For', '192.168.1.1')
                .expect(200);
            const ip2Response = await (0, supertest_1.default)(testApp.app)
                .get('/api/test')
    ...
    describe('Different endpoints have appropriate limits', () => {
        ...
        test('should block after 10 requests on strict endpoints', async () => {
            const requests = Array.from({ length: 10 }, () => (0, supertest_1.default)(testApp.app)
                .post('/api/auth/login')
                .send({ username: 'test', password: 'test' }));
            await Promise.all(requests);
            const blockedResponse = await (0, supertest_1.default)(testApp.app)
                .post('/api/auth/login')
                .send({ username: 'test', password: 'test' })
                .expect(429);
        ...
        test('should not rate limit health endpoint', async () => {
            const requests = Array.from({ length: 15 }, () => (0, supertest_1.default)(testApp.app).get('/health'));
            const responses = await Promise.all(requests);
    ...
    describe('Rate limit window behavior', () => {
        test('should maintain separate windows for different endpoint categories', async () => {
            await (0, supertest_1.default)(testApp.app).get('/api/test').expect(200);
            const authResponse = await (0, supertest_1.default)(testApp.app)
                .post('/api/auth/login')
                .send({ username: 'test' })
                .expect(200);
        ...
        test('should handle concurrent requests correctly', async () => {
            const concurrentRequests = Array.from({ length: 25 }, () => (0, supertest_1.default)(testApp.app).get('/api/test'));
            const responses = await Promise.all(concurrentRequests);
            responses.forEach(response => {
                expect(response.status).toBe(200);
                expect(parseInt(response.headers['x-ratelimit-limit'])).toBe(100);
            });
            const finalResponse = await (0, supertest_1.default)(testApp.app)
                .get('/api/test')
                .expect(200);
    ...
    describe('Edge cases and error scenarios', () => {
        ...
        test('should handle various HTTP methods consistently', async () => {
            for (const method of ['get', 'post']) {
                const response = await (0, supertest_1.default)(testApp.app)[method]('/api/test')
