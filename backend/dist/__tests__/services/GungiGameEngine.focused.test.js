...
describe('Gungi Game Engine - Core Tests', () => {
    let gameEngine;
    beforeEach(() => {
        gameEngine = new GungiGameEngine_1.GungiGameEngine('test-game');
    });
    ...
    describe('3. Move Validation', () => {
        test('should reject invalid moves', () => {
            const result = gameEngine.makeMove({ row: 0, col: 0, tier: 0 }, { row: 8, col: 8, tier: 0 });
    ...
    describe('6. Serialization', () => {
        test('should serialize and deserialize game state', () => {
            ...
            const newEngine = new GungiGameEngine_1.GungiGameEngine('test-deserialization');
    ...
    describe('7. Performance and Edge Cases', () => {
        ...
        test('should handle empty position queries gracefully', () => {
            const result = gameEngine.makeMove({ row: 4, col: 4, tier: 0 }, { row: 5, col: 5, tier: 0 });
            expect(result.success).toBe(false);
            expect(result.error).toContain('No piece');
        });
    });
});
//# sourceMappingURL=GungiGameEngine.focused.test.js.map