...
const OptimizedBettingService_1 = require("../../services/OptimizedBettingService");
...
describe('Betting Core Functionality', () => {
    ...
    beforeAll(() => {
        ...
        optimizedBettingService = new OptimizedBettingService_1.OptimizedBettingService();
    });
    beforeEach(() => {
        jest.clearAllMocks();
        process.env.MIN_BET_LAMPORTS = (minBetAmount * web3_js_1.LAMPORTS_PER_SOL).toString();
        process.env.MAX_BET_LAMPORTS = (maxBetAmount * web3_js_1.LAMPORTS_PER_SOL).toString();
    ...
    describe('Valid bet placement succeeds', () => {
        test('should successfully place a valid bet with EnhancedBettingService', async () => {
            ...
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            ...
            expect(betData.amount).toBe(validBetAmount * web3_js_1.LAMPORTS_PER_SOL);
        ...
        test('should successfully place a bet with OptimizedBettingService', async () => {
            const result = await optimizedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount, mockAgentId, 'ai_agent');
        ...
        test('should return success response with proper structure', async () => {
            ...
            requiredFields.forEach(field => {
                ...
                expect(betData[field]).toBeDefined();
            });
        });
        test('should generate unique bet IDs for multiple bets', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const bet1 = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            const bet2 = await enhancedBettingService.placeBet('different-wallet-address', mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, 'phantom_striker', 'ai_agent');
    ...
    describe('Bet amount validation (0.1-100 SOL)', () => {
        test('should accept minimum valid bet amount (0.1 SOL)', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, minBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(betData.amount).toBe(minBetAmount * web3_js_1.LAMPORTS_PER_SOL);
        });
        test('should accept maximum valid bet amount (100 SOL)', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, maxBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(betData.amount).toBe(maxBetAmount * web3_js_1.LAMPORTS_PER_SOL);
        });
        test('should reject bet amount below minimum (0.09 SOL)', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            await expect(enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, 0.09 * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent')).rejects.toThrow(/Bet amount must be between/);
        });
        test('should reject bet amount above maximum (101 SOL)', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            await expect(enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, 101 * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent')).rejects.toThrow(/Bet amount must be between/);
        });
        test('should handle edge case amounts correctly', async () => {
            const result = await optimizedBettingService.placeBet(mockBettorWallet, mockMatchId, 0.05, mockAgentId, 'ai_agent');
            expect(result.success).toBe(false);
            expect(result.error).toContain('must be between');
        });
        test('should validate amount precision and type', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const preciseAmount = 1.123456789 * web3_js_1.LAMPORTS_PER_SOL;
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, Math.floor(preciseAmount), mockAgentId, 'ai_agent');
    ...
    describe('Match status validation for betting', () => {
        ...
        test('should reject betting on non-existent match', async () => {
            ...
            await expect(enhancedBettingService.placeBet(mockBettorWallet, nonExistentMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent')).rejects.toThrow(/Betting pool not found/);
        });
        test('should reject betting on inactive/closed matches', async () => {
            const pool = await enhancedBettingService.createBettingPool(mockMatchId);
            const activePools = enhancedBettingService.activePools;
            if (activePools.has(mockMatchId)) {
                activePools.get(mockMatchId).isActive = false;
            }
            await expect(enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent')).rejects.toThrow(/Betting is closed/);
        });
        test('should handle match status transitions correctly', async () => {
            const result1 = await optimizedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount, mockAgentId, 'ai_agent');
            expect(result1.success).toBe(true);
            const activePools = optimizedBettingService.activePools;
            if (activePools.has(mockMatchId)) {
                activePools.get(mockMatchId).isActive = false;
            }
            const result2 = await optimizedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount, mockAgentId, 'ai_agent');
    ...
    describe('User balance verification', () => {
        test('should validate sufficient balance before bet placement', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const highAmount = 1000 * web3_js_1.LAMPORTS_PER_SOL;
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(betData).toBeDefined();
        });
        test('should handle balance check failures gracefully', async () => {
            const result = await optimizedBettingService.placeBet(mockBettorWallet, mockMatchId, 1000, mockAgentId, 'ai_agent');
            expect(result).toBeDefined();
            expect(typeof result.success).toBe('boolean');
        });
        test('should reserve bet amount from user balance', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const initialBalance = 10 * web3_js_1.LAMPORTS_PER_SOL;
            const betAmount = 2 * web3_js_1.LAMPORTS_PER_SOL;
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, betAmount, mockAgentId, 'ai_agent');
    ...
    describe('Duplicate bet prevention', () => {
        test('should allow multiple bets from same user on different matches', async () => {
            ...
            const bet1 = await enhancedBettingService.placeBet(mockBettorWallet, matchId1, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            const bet2 = await enhancedBettingService.placeBet(mockBettorWallet, matchId2, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(bet1.id).not.toBe(bet2.id);
        ...
        test('should allow multiple bets from different users on same match', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const bet1 = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            const bet2 = await enhancedBettingService.placeBet('different-wallet-123', mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, 'phantom_striker', 'ai_agent');
            expect(bet1.id).not.toBe(bet2.id);
        ...
        test('should allow same user to bet on different agents in same match', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const bet1 = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, 'royal_guard_alpha', 'ai_agent');
            const bet2 = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, 'phantom_striker', 'ai_agent');
            expect(bet1.id).not.toBe(bet2.id);
    ...
    describe('Bet record creation in database', () => {
        test('should create bet record with all required fields', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(betData.id).toMatch(/^[0-9a-f-]{36}$/);
            expect(betData.matchId).toBe(mockMatchId);
            expect(betData.bettorWallet).toBe(mockBettorWallet);
        ...
        test('should handle concurrent bet record creation', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const promises = Array.from({ length: 5 }, (_, i) => enhancedBettingService.placeBet(`wallet-${i}`, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent'));
    ...
    describe('Betting pool updates in Redis', () => {
        test('should update pool totals after bet placement', async () => {
            ...
            await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            const updatedPool = await enhancedBettingService.getBettingPool(mockMatchId);
            expect(updatedPool).not.toBeNull();
            expect(updatedPool.totalPool).toBe(initialTotal + (validBetAmount * web3_js_1.LAMPORTS_PER_SOL));
        });
        ...
        test('should maintain pool integrity across multiple bets', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betAmounts = [1 * web3_js_1.LAMPORTS_PER_SOL, 2 * web3_js_1.LAMPORTS_PER_SOL, 0.5 * web3_js_1.LAMPORTS_PER_SOL];
            const expectedTotal = betAmounts.reduce((sum, amount) => sum + amount, 0);
            for (let i = 0; i < betAmounts.length; i++) {
                await enhancedBettingService.placeBet(`wallet-${i}`, mockMatchId, betAmounts[i], mockAgentId, 'ai_agent');
            }
    ...
    describe('Odds calculation accuracy', () => {
        test('should calculate initial odds correctly (default 2.0)', async () => {
            ...
            expect(betData.odds).toBeLessThanOrEqual(10.0);
        });
        test('should update odds based on betting pool distribution', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const bet1 = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, 1 * web3_js_1.LAMPORTS_PER_SOL, 'agent1', 'ai_agent');
            const bet2 = await enhancedBettingService.placeBet('wallet-2', mockMatchId, 3 * web3_js_1.LAMPORTS_PER_SOL, 'agent2', 'ai_agent');
            expect(bet1.odds).not.toBe(bet2.odds);
        });
        test('should maintain odds within reasonable bounds', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betData = await enhancedBettingService.placeBet(mockBettorWallet, mockMatchId, validBetAmount * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent');
            expect(betData.odds).toBeGreaterThanOrEqual(1.01);
            expect(betData.odds).toBeLessThanOrEqual(10.0);
        });
    ...
    describe('Bet confirmation responses', () => {
        test('should return comprehensive bet confirmation', async () => {
            ...
            expect(betData).toMatchObject({
                ...
                amount: validBetAmount * web3_js_1.LAMPORTS_PER_SOL,
    ...
    describe('Performance and Edge Cases', () => {
        test('should handle high-frequency bet placement', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const promises = Array.from({ length: 100 }, (_, i) => enhancedBettingService.placeBet(`high-freq-wallet-${i}`, mockMatchId, 0.1 * web3_js_1.LAMPORTS_PER_SOL, mockAgentId, 'ai_agent').catch((error) => ({ error: error.message })));
            const results = await Promise.all(promises);
        ...
        test('should maintain data consistency under load', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const betAmount = 1 * web3_js_1.LAMPORTS_PER_SOL;
            const numberOfBets = 10;
            const promises = Array.from({ length: numberOfBets }, (_, i) => enhancedBettingService.placeBet(`load-test-wallet-${i}`, mockMatchId, betAmount, mockAgentId, 'ai_agent'));
        ...
        test('should handle invalid inputs gracefully', async () => {
            await enhancedBettingService.createBettingPool(mockMatchId);
            const invalidTests = [
                { wallet: null, amount: validBetAmount * web3_js_1.LAMPORTS_PER_SOL },
                { wallet: '', amount: validBetAmount * web3_js_1.LAMPORTS_PER_SOL },
                { wallet: mockBettorWallet, amount: null },
                { wallet: mockBettorWallet, amount: 'invalid' }
            ];
            for (const test of invalidTests) {
                try {
                    await enhancedBettingService.placeBet(test.wallet, mockMatchId, test.amount, mockAgentId, 'ai_agent');
                }
                catch (error) {
...
//# sourceMappingURL=betting-core.test.js.map