...
function getGameService() {
    ...
    try {
        ...
        logger_1.logger.error('Failed to initialize GameService', {
            error: error instanceof Error ? error.message : String(error)
        });
        throw (0, errorHandler_1.createError)('Game service unavailable', 503);
    }
}
...
router.get('/board/:gameId', async (req, res, next) => {
    try {
        ...
        if (!gameState) {
            throw (0, errorHandler_1.createError)('Game not found', 404);
        }
        res.json({
            success: true,
            board: gameState
...
router.post('/move', async (req, res, next) => {
    try {
        ...
        if (!gameId || !from || !to || !piece) {
            throw (0, errorHandler_1.createError)('Missing required fields: gameId, from, to, piece', 400);
        }
