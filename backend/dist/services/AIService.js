...
class AIService {
    ...
    async runTrainingSession(session) {
      ...
      `, [new Date(), session.id]);
            ...
            const eloImprovement = Math.floor(Math.random() * 50) + 10;
    ...
    calculateTrainingCost(episodes) {
        const baseCostPerEpisode = 0.0001;
    ...
    async updateAgentElo(agentId, result, opponentElo) {
        try {
            ...
            const newElo = Math.max(100, agent.eloRating + eloChange);
