...
class AdvancedLoadTestingService {
    ...
    async capturePerformanceSnapshot() {
        ...
        const recentSessions = Array.from(this.activeSessions.values())
            .filter(s => s.endTime && (Date.now() - s.endTime) < 30000);
    ...
    async stressDatabaseConnections(config) {
        ...
        const numConnections = Math.min(config.maxConcurrentGames / 10, 100);
    ...
    async stressWebSocketConnections(config) {
        ...
        const numConnections = Math.min(config.maxConcurrentGames, 500);
        const connectionPromises = [];
        for (let i = 0; i < numConnections; i++) {
    ...
    async executeConcurrentGameTest(config) {
        ...
        const gamesPerBatch = Math.ceil(config.maxConcurrentGames / (config.rampUpTimeMs / 5000));
        let gamesStarted = 0;
    ...
    async simulateGame(gameId) {
        ...
        try {
            const gameDuration = Math.random() * 30000 + 10000;
            const numMoves = Math.floor(Math.random() * 50 + 20);
            ...
            if (this.redisClient) {
                await this.redisClient.setEx(`load_test_result:${sessionId}`, 3600, JSON.stringify({
                    sessionId,
    ...
    async simulateDatabaseWrite(sessionId, moveNumber) {
        try {
            ...
            if (this.redisClient) {
                await this.redisClient.setEx(`move:${sessionId}:${moveNumber}`, 300, JSON.stringify(moveData));
            }
        }
        catch (error) {
    ...
    async calculateFinalMetrics() {
        ...
        this.metrics.throughput = (this.metrics.gamesCompleted / testDuration) * 1000;
