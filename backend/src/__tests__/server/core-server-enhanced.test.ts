/**
 * Enhanced Core Server Functionality Tests - Production-Grade Implementation
...
describe('Core Server Functionality', () => {
    ...
    describe('2. Express app initializes with all middleware', () => {
        ...
        test('Request size limits are enforced', async () => {
            // Test with large payload - create a reasonably large but not excessive payload
            const largeData = {
                data: 'x'.repeat(1024 * 100) // 100KB - reasonable size for testing
            };
            ...
            expect([200, 404, 413, 415]).toContain(response.status);
        });
    });
    ...
    describe('Middleware Performance', () => {
        test('Middleware processing performance meets requirements', async () => {
            ...
            console.log(`⚡ Middleware processing - Average: ${averageTime.toFixed(2)}ms, P95: ${p95Time.toFixed(2)}ms`);
        });

        test('Compression middleware performance', async () => {
            // Test compression middleware with larger response
            const startTime = performance.now();
            const response = await request(app)
                .get('/api-docs')
                .expect(200);
            ...
            const responseTime = endTime - startTime;
            expect(responseTime).toBeLessThan(20); // Should be fast even with compression

            // Verify response is structured (successfully processed)
    ...
    describe('API Documentation and Metrics', () => {
        ...
        test('Metrics endpoint provides comprehensive system information', async () => {
            const response = await request(app)
                .get('/metrics')
                .expect(200);
    ...
    describe('Production Configuration Validation', () => {
        ...
        test('Security configurations are active in responses', async () => {
            const response = await request(app)
                .get('/health');
        ...
        test('Server handles load within performance requirements', async () => {
