...
export interface Position {
  row: number; // 0-8
  col: number; // 0-8
  tier: number; // 0-2 (stacking level)
}
...
const GameStateSchema = z.object({
  id: z.string(),
  board: z.array(z.array(z.array(PieceSchema.nullable()))).length(9),
  currentPlayer: z.nativeEnum(Player),
  status: z.nativeEnum(GameStatus),
  result: z.nativeEnum(GameResult),
  moves: z.array(MoveSchema),
  capturedPieces: z.array(PieceSchema),
  startTime: z.number(),
  endTime: z.number().optional(),
  winner: z.nativeEnum(Player).optional()
});
...
export class GungiGameEngine {
  ...
  private initializeGame(gameId: string): GameState {
    const board: (Piece | null)[][][] = Array(9).fill(null).map(() =>
   ...
   */
  public makeMove(from: Position, to: Position): { success: boolean; error?: string; move?: Move } {
    try {
      // Validate positions
   ...
   */
  private validateMove(piece: Piece, from: Position, to: Position): { isValid: boolean; reason?: string } {
   ...
   */
  private validatePieceMovement(piece: Piece, from: Position, to: Position): { isValid: boolean; reason?: string } {
   ...
   */
  private executeMoveInternal(piece: Piece, from: Position, to: Position): Move {
   ...
   */
  private checkAdvancedEndConditions(): void {
   ...
   */
  private findPieceByType(player: Player, pieceType: PieceType): Piece | null {
  ...
  public getValidMoves(): Move[] {
    ...
    currentPlayerPieces.forEach(piece => {
      ...
      for (let row = 0; row < 9; row++) {
        for (let col = 0; col < 9; col++) {
          for (let tier = 0; tier < 3; tier++) {
            const to: Position = { row, col, tier };
   ...
   */
  private getAllPiecesForPlayer(player: Player): Piece[] {
    const pieces: Piece[] = [];
  ...
  public getBoardRepresentation(): string[][] {
    const boardRep: string[][] = Array(9).fill(null).map(() => Array(9).fill('.'));
   ...
   */
  public importGameState(gameStateJson: string): boolean {
    try {
   ...
   */
  public setGameStatus(status: GameStatus, result?: GameResult, winner?: Player, endTime?: number): void {
