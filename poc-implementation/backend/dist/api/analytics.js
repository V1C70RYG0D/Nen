...
router.post('/web-vitals', async (req, res) => {
    try {
        ...
        server_1.logger.info('Core Web Vital received:', {
            name: metric.name,
            value: metric.value,
        ...
        */
        ...
        if (metric.rating === 'poor') {
            server_1.logger.warn(`Poor performance detected: ${metric.name} = ${metric.value}`, {
                sessionId: metric.sessionId,
                userId: metric.userId,
        ...
        server_1.logger.error('Error processing web vital metric:', error);
        res.status(500).json({
            success: false,
...
router.get('/web-vitals', async (req, res) => {
    try {
        ...
        server_1.logger.info(`Fetching web vitals analytics`, { timeRange, metricType, userId, sessionId });
        ...
        server_1.logger.error('Error fetching web vitals analytics:', error);
        res.status(500).json({
            success: false,
