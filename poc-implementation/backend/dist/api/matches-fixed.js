...
router.get('/', async (req, res) => {
    try {
        ...
        logger_1.logger.info('Providing demo matches for User Story 3 demonstration');
        ...
        if (minRating || maxRating) {
            filteredMatches = filteredMatches.filter((match) => {
                ...
                if (minRating && avgRating < parseInt(minRating))
                    return false;
                if (maxRating && avgRating > parseInt(maxRating))
                    return false;
                return true;
            });
        }
        if (minBet || maxBet) {
            filteredMatches = filteredMatches.filter((match) => {
                if (minBet && match.bettingPoolSol < parseFloat(minBet))
                    return false;
                if (maxBet && match.bettingPoolSol > parseFloat(maxBet))
                    return false;
                return true;
            });
        }
...
//# sourceMappingURL=matches-fixed.js.map