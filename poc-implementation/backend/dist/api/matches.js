...
function setGameService(service) {
...
router.get('/', async (req, res) => {
    try {
        logger_1.logger.info('User Story 3: Providing real devnet matches with filtering capabilities');
        ...
        const { status, minRating, maxRating, minBet, maxBet, minBetRange, maxBetRange, minAiRating, maxAiRating, search, nenTypes, sortBy = 'startTime', sortOrder = 'desc', page = 1, limit = 20 } = req.query;
        ...
        if (search && typeof search === 'string') {
            ...
            filteredMatches = filteredMatches.filter((match) => match.agent1.name.toLowerCase().includes(searchTerm) ||
                match.agent2.name.toLowerCase().includes(searchTerm));
        }
        // Nen type filter
        if (nenTypes) {
            const nenTypeArray = Array.isArray(nenTypes) ? nenTypes : [nenTypes];
            filteredMatches = filteredMatches.filter((match) => nenTypeArray.includes(match.agent1.nenType) ||
        ...
        if (minAiRatingNum || maxAiRatingNum) {
            filteredMatches = filteredMatches.filter((match) => {
                ...
                if (minAiRatingNum && avgRating < minAiRatingNum)
                    return false;
                if (maxAiRatingNum && avgRating > maxAiRatingNum)
                    return false;
                return true;
            });
        }
        ...
        if (minBetNum || maxBetNum) {
            filteredMatches = filteredMatches.filter((match) => {
                if (minBetNum && match.bettingPoolSol < minBetNum)
                    return false;
                if (maxBetNum && match.bettingPoolSol > maxBetNum)
                    return false;
                return true;
            });
        }
...
router.get('/:matchId', async (req, res) => {
    try {
        ...
        const gameServiceInstance = (0, ServiceFactory_1.getGameServiceInstance)();
        ...
        let bettingPool;
        try {
            bettingPool = await ServiceFactory_1.bettingService.getBettingPool(matchId);
        }
        catch (error) {
