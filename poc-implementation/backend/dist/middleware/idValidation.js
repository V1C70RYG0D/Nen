...
exports.validationService = exports.validateAnyID = exports.validateMatchID = void 0;
const logger_1 = require("../utils/logger");
const uuid_1 = require("uuid");
class IDValidationService {
    constructor() {
        this.suspiciousRequests = {};
        this.MAX_INVALID_REQUESTS = 10;
    ...
    validateID(id) {
        ...
        if (!(0, uuid_1.validate)(decodedId)) {
     ...
     */
    trackSuspiciousRequest(ip, pattern) {
        const now = new Date();
...
const validateMatchID = (req, res, next) => {
    ...
    if (!validation.isValid) {
        ...
        logger_1.logger.warn('Invalid match ID request', {
            matchId,
        ...
        if (validation.suspiciousActivity) {
            const isRateLimited = validationService.trackSuspiciousRequest(clientIP, validation.activityType || 'unknown');
            ...
            if (isRateLimited) {
                logger_1.logger.error('Rate limit exceeded for suspicious requests', {
                    ip: clientIP,
                    userAgent,
                    timestamp: new Date().toISOString()
                });
...
exports.validateMatchID = validateMatchID;
/**
...
const validateAnyID = (paramName) => {
    return (req, res, next) => {
        ...
        const userAgent = req.get('User-Agent') || 'unknown';
        if (!validation.isValid) {
            logger_1.logger.warn(`Invalid ${paramName} request`, {
                [paramName]: id,
                error: validation.error,
                ip: clientIP,
                userAgent,
...
exports.validateAnyID = validateAnyID;
