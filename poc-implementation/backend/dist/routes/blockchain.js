...
router.get('/balance/:address', async (req, res) => {
    try {
        ...
        if (cachedBalance !== null) {
            logger_1.logger.debug('Returning cached balance', { address, balance: cachedBalance });
            return res.json({
        ...
        logger_1.logger.info('Balance retrieved', { address, balance, lamports });
        res.json(result);
    }
    catch (error) {
...
router.get('/transactions/:address', async (req, res) => {
    try {
        ...
        // Validate limit
        const transactionLimit = Math.min(Math.max(parseInt(limit) || 20, 1), 100);
        // Check cache first
        const cacheKey = `transactions:${address}:${transactionLimit}:${before || 'latest'}`;
        const cachedTransactions = await redis.get(cacheKey);
        if (cachedTransactions !== null) {
            logger_1.logger.debug('Returning cached transactions', { address, count: cachedTransactions.transactions.length });
            return res.json({
                ...cachedTransactions,
                cached: true
            });
        ...
        const options = { limit: transactionLimit };
        if (before) {
        ...
        logger_1.logger.info('Transaction history retrieved', { address, count: transactions.length });
        res.json(result);
    }
    catch (error) {
        logger_1.logger.error('Error getting transactions:', error);
...
router.get('/transaction/:signature', async (req, res) => {
    try {
        ...
        if (cachedTransaction !== null) {
            logger_1.logger.debug('Returning cached transaction', { signature });
            return res.json({
                ...cachedTransaction,
                cached: true
            });
        ...
        logger_1.logger.info('Transaction details retrieved', { signature });
        res.json(result);
    }
    catch (error) {
        logger_1.logger.error('Error getting transaction details:', error);
...
router.get('/network/status', async (req, res) => {
    try {
        ...
        if (cachedStatus !== null) {
            logger_1.logger.debug('Returning cached network status');
            return res.json({
                ...cachedStatus,
                cached: true
            });
        ...
        logger_1.logger.info('Network status retrieved', { slot, blockHeight });
        res.json(result);
    }
    catch (error) {
...
router.post('/validate/address', async (req, res) => {
    try {
        ...
        try {
            ...
            isValid = true;
            isOnCurve = web3_js_1.PublicKey.isOnCurve(publicKey);
        }
        catch (error) {
            isValid = false;
        }
        ...
        logger_1.logger.info('Address validated', { address, valid: isValid, isOnCurve });
        res.json(result);
    }
    catch (error) {
        logger_1.logger.error('Error validating address:', error);
...
router.get('/health', async (req, res) => {
    try {
        ...
        logger_1.logger.error('Blockchain health check failed:', error);
        res.status(503).json({
            status: 'unhealthy',
...
router.use((error, req, res, next) => {
    logger_1.logger.error('Blockchain route error:', error);
    res.status(500).json({
