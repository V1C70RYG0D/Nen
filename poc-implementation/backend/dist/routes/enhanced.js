...
const EnhancedAICustomizationService_1 = require("../services/EnhancedAICustomizationService");
const EnhancedBettingServiceV2_1 = require("../services/EnhancedBettingServiceV2");
const EnhancedNFTAgentService_1 = require("../services/EnhancedNFTAgentService");
...
router.get('/ai/agents', async (req, res) => {
    ...
    try {
        const agents = await EnhancedAICustomizationService_1.enhancedAICustomizationService.getAllAgents();
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`AI agents retrieved in ${responseTime.toFixed(2)}ms`);
        return res.json({
            success: true,
            data: agents,
            metadata: {
        ...
        logger_1.logger.error('Failed to retrieve AI agents:', error);
...
router.post('/ai/agents', async (req, res) => {
    ...
    try {
        ...
        const agent = await EnhancedAICustomizationService_1.enhancedAICustomizationService.createCustomAIAgent(name, personality, difficulty, customizations, userId);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`Custom AI agent created in ${responseTime.toFixed(2)}ms`);
...
router.post('/ai/agents/:agentId/move', async (req, res) => {
    ...
    try {
        ...
        const moveAnalysis = await EnhancedAICustomizationService_1.enhancedAICustomizationService.generateEnhancedMove(agentId, gameState, timeLimit || 5000);
        const responseTime = performance.now() - startTime;
        // Validate POC requirement: <50ms for moves
        if (responseTime > 50) {
            logger_1.logger.warn(`Move generation exceeded 50ms target: ${responseTime.toFixed(2)}ms`);
        }
        return res.json({
            success: true,
            data: moveAnalysis,
        ...
        logger_1.logger.error(`Failed to generate AI move for agent ${req.params.agentId}:`, error);
        return res.status(500).json({
            success: false,
...
router.post('/ai/agents/:agentId/train', async (req, res) => {
    ...
    try {
        ...
        const evolutionPoint = await EnhancedAICustomizationService_1.enhancedAICustomizationService.trainAgent(agentId, trainingGames || 100, opponentAgents || []);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`AI agent training completed in ${responseTime.toFixed(2)}ms`);
        return res.json({
            success: true,
            data: evolutionPoint,
            metadata: {
        ...
        logger_1.logger.error(`Failed to train AI agent ${req.params.agentId}:`, error);
        return res.status(500).json({
            success: false,
...
router.post('/betting/pools', async (req, res) => {
    ...
    try {
        ...
        const pool = await EnhancedBettingServiceV2_1.enhancedBettingService.createBettingPool(matchId);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`Betting pool created in ${responseTime.toFixed(2)}ms`);
        return res.status(201).json({
            success: true,
            data: pool,
            metadata: {
        ...
        logger_1.logger.error('Failed to create betting pool:', error);
        return res.status(500).json({
            success: false,
...
router.post('/betting/bets', async (req, res) => {
    const startTime = performance.now();
    try {
        const { matchId, bettorWallet, amount, predictedWinner, predictedWinnerType, geoLocation } = req.body;
        ...
        const bet = await EnhancedBettingServiceV2_1.enhancedBettingService.placeBet(matchId, bettorWallet, amount, predictedWinner, predictedWinnerType, geoLocation);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`Bet placed in ${responseTime.toFixed(2)}ms`);
        return res.status(201).json({
            success: true,
            data: bet,
            metadata: {
        ...
        logger_1.logger.error('Failed to place bet:', error);
        return res.status(500).json({
            success: false,
...
router.get('/betting/pools/:matchId', async (req, res) => {
    ...
    try {
        ...
        const pool = await EnhancedBettingServiceV2_1.enhancedBettingService.getBettingPool(matchId);
        if (!pool) {
        ...
        logger_1.logger.error(`Failed to retrieve betting pool ${req.params.matchId}:`, error);
        return res.status(500).json({
            success: false,
...
router.post('/nft/mint', async (req, res) => {
    ...
    try {
        ...
        const nft = await EnhancedNFTAgentService_1.enhancedNFTAgentService.mintEnhancedAIAgentNFT(baseAgent, creatorWallet, specialAbilities || [], customMetadata);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`Enhanced NFT minted in ${responseTime.toFixed(2)}ms`);
        return res.status(201).json({
            success: true,
            data: nft,
            metadata: {
        ...
        logger_1.logger.error('Failed to mint enhanced NFT:', error);
        return res.status(500).json({
            success: false,
...
router.post('/nft/:nftId/evolve', async (req, res) => {
    ...
    try {
        ...
        const evolvedNFT = await EnhancedNFTAgentService_1.enhancedNFTAgentService.evolveAgentNFT(nftId, performanceData, newAbilities || []);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`NFT evolved in ${responseTime.toFixed(2)}ms`);
        return res.json({
            success: true,
            data: evolvedNFT,
            metadata: {
        ...
        logger_1.logger.error(`Failed to evolve NFT ${req.params.nftId}:`, error);
        return res.status(500).json({
            success: false,
...
router.post('/nft/:nftId/list', async (req, res) => {
    ...
    try {
        ...
        const listing = await EnhancedNFTAgentService_1.enhancedNFTAgentService.listOnMarketplace(nftId, price, marketplace);
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`NFT listed in ${responseTime.toFixed(2)}ms`);
        return res.json({
            success: true,
            data: listing,
            metadata: {
        ...
        logger_1.logger.error(`Failed to list NFT ${req.params.nftId}:`, error);
        return res.status(500).json({
            success: false,
...
router.get('/nft/marketplace/stats', async (req, res) => {
    const startTime = performance.now();
    try {
        const stats = await EnhancedNFTAgentService_1.enhancedNFTAgentService.getMarketplaceStats();
        const responseTime = performance.now() - startTime;
        return res.json({
            success: true,
            data: stats,
        ...
        logger_1.logger.error('Failed to retrieve marketplace stats:', error);
        return res.status(500).json({
            success: false,
...
router.get('/cache/metrics', async (req, res) => {
    const startTime = performance.now();
    try {
        const metrics = EnhancedCachingService_1.enhancedCachingService.getCacheMetrics();
        const responseTime = performance.now() - startTime;
        return res.json({
            success: true,
            data: metrics,
        ...
        logger_1.logger.error('Failed to retrieve cache metrics:', error);
        return res.status(500).json({
            success: false,
...
router.post('/cache/warmup', async (req, res) => {
    const startTime = performance.now();
    try {
        await EnhancedCachingService_1.enhancedCachingService.warmupCache();
        const responseTime = performance.now() - startTime;
        logger_1.logger.info(`Cache warmup completed in ${responseTime.toFixed(2)}ms`);
        return res.json({
            success: true,
        ...
        logger_1.logger.error('Failed to warmup cache:', error);
        return res.status(500).json({
            success: false,
