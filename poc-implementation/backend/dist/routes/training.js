...
router.post('/sessions', async (req, res) => {
    ...
    try {
        ...
        try {
            await devnet.validateCidAvailability(finalCid);
        }
        catch (_) { }
        ...
        logger_1.logger.info('NEN stake validation', {
            wallet: walletPubkey,
        ...
        logger_1.logger.info('Training session initiated', {
        ...
        logger_1.logger.apiError('/api/v1/training/sessions', error, { durationMs: Date.now() - startedAt });
...
router.get('/sessions/:id', async (req, res) => {
    try {
        ...
        logger_1.logger.apiError('/api/v1/training/sessions/:id', error);
        res.status(500).json({ success: false, error: error?.message || 'Internal error' });
    }
});
...
router.get('/nen-status', async (req, res) => {
    try {
        ...
        logger_1.logger.apiError('/api/v1/training/nen-status', error);
        res.status(500).json({ success: false, error: error?.message || 'Internal error' });
    }
});
...
router.post('/validate-stake', async (req, res) => {
    try {
        ...
        logger_1.logger.apiError('/api/v1/training/validate-stake', error);
        res.status(500).json({ success: false, error: error?.message || 'Internal error' });
    }
});
...
router.post('/complete', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.warn('NotificationService unavailable or failed', { error: e.message });
        }
        ...
        try {
            ...
            await registry.publishMatch({ id: practiceId, status: 'scheduled', agents: [agentMint, agentMint], scheduledAt: new Date(Date.now() + 60000).toISOString() });
        }
        catch (e) {
            logger_1.logger.warn('Failed to publish practice match memo', { error: e.message });
        }
        res.json({
            success: true,
        ...
        logger_1.logger.apiError('/api/v1/training/complete', error, { durationMs: Date.now() - startedAt });
        res.status(500).json({ success: false, error: error?.message || 'Internal error' });
    }
});
