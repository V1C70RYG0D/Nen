...
import { getEnhancedComplianceService } from '../services/EnhancedComplianceService';
...
let bettingServiceInstance: any = null;
...
function getBettingService() {
  ...
  try {
    ...
    logger.error('Failed to initialize BettingService', {
      error: error instanceof Error ? error.message : String(error)
    });
    throw createError('Betting service unavailable', 503);
  }
}

// Export function to set service instance (for dependency injection)
export function setBettingService(service: any) {
...
router.post('/place', async (req, res, next) => {
  try {
    ...
    if (fraudResult.recommendedAction === 'block') {
      logger.warn('Bet blocked due to fraud detection', {
    ...
    if (fraudResult.recommendedAction === 'review') {
      logger.info('Bet requires review due to moderate risk', {
        walletAddress: bettorWallet,
        amount,
        riskScore: fraudResult.riskScore
      });
...
router.post('/settle/:matchId', async (req, res, next) => {
  try {
    ...
    if (!winnerId) {
      throw createError('Missing required field: winnerId', 400);
    }
...
router.get('/compliance/metrics', async (req, res, next) => {
  try {
    ...
    logger.error('Failed to retrieve compliance metrics', {
      error: error instanceof Error ? error.message : String(error)
    });
    next(error);
  }
...
router.get('/patterns/:walletAddress', async (req, res, next) => {
  try {
    ...
    logger.error('Failed to analyze transaction patterns', {
