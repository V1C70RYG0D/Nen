    ...
    this.io.on('connection', (socket: AuthenticatedSocket) => {
      ...
      socket.on('join-game', async (data) => {
        try {
          ...
          logger.info('User joined game', { socketId: socket.id, gameId, userId });
        } catch (error) {
      ...
      socket.on('leave-game', async (data) => {
        try {
          ...
          logger.info('User left game', { socketId: socket.id, gameId, userId });
        } catch (error) {
      ...
      socket.on('game-move', async (data) => {
        try {
          ...
          logger.info('Game move processed', { socketId: socket.id, gameId, userId, move });
        } catch (error) {
      ...
      socket.on('chat-message', async (data) => {
        try {
          ...
          logger.info('Chat message sent', { socketId: socket.id, gameId, userId });
        } catch (error) {
      ...
      socket.on('place-bet', async (data) => {
        try {
          ...
          logger.info('Bet placed', { socketId: socket.id, gameId, userId, amount, betType });
        } catch (error) {
      ...
      socket.on('game-update', async (data) => {
        try {
          ...
          logger.info('Game update sent', { socketId: socket.id, gameId, userId });
        } catch (error) {
      ...
      socket.on('disconnect', (reason) => {
        ...
        if (connection) {
          ...
          connection.rooms.forEach((room: string) => {
...
router.get('/stats', async (req, res) => {
  try {
    const stats = {
      ...
      connectedUsers: websocketManager.getConnections(),
...
router.post('/broadcast', async (req, res) => {
  try {
    ...
    websocketManager.broadcast(event, data, room);
    ...
    res.json(result);
    logger.info('Message broadcasted', { event, room: room || 'all' });
  } catch (error) {
...
router.get('/room/:roomId/stats', async (req, res) => {
  try {
    ...
    const stats = {
      roomId,
      connectionCount: websocketManager.getRoomCount(roomId),
      timestamp: new Date().toISOString()
    };

    res.json(stats);
    logger.info('Room stats requested', { roomId });
  } catch (error) {
...
router.get('/chat/:gameId/history', async (req, res) => {
  try {
    ...
    const messageLimit = Math.min(Math.max(parseInt(limit as string) || 50, 1), 100);
    ...
    logger.info('Chat history retrieved', { gameId, count: parsedMessages.length });
  } catch (error) {
