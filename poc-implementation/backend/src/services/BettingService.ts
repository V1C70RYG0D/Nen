...
export class BettingService {
  ...
  constructor() {
    ...
    this.programId = new PublicKey(
      process.env.NEN_BETTING_PROGRAM_ID || 'C8uJ3ABMU87GjcB8moR1jiiAgYnFUDR17DBfiQE4eUcz'
    );
  }
  ...
  async getBettingAccount(walletAddress: string): Promise<BettingAccount> {
    try {
      ...
      const [bettingAccountPda] = PublicKey.findProgramAddressSync(
        [Buffer.from('betting_account'), walletPubkey.toBuffer()],
        this.programId
      );
  ...
  const pdaAddress = new PublicKey(bettingAccount.pdaAddress);
      ...
      const keys = tx.transaction.message.getAccountKeys().staticAccountKeys || (tx.transaction.message as any).accountKeys;
      const pre = tx.meta.preBalances;
      const post = tx.meta.postBalances;
      const pdaIndex = keys.findIndex((k: any) => k.toString ? k.toString() === pdaAddress.toString() : k === pdaAddress.toString());
      const userIndex = keys.findIndex((k: any) => k.toString ? k.toString() === walletAddress : k === walletAddress);
  ...
  async withdrawSol(withdrawalRequest: WithdrawalRequest): Promise<WithdrawalResult> {
    try {
      ...
      const txSignature = (withdrawalRequest as any).transactionSignature as string | undefined;
      if (!txSignature) {
