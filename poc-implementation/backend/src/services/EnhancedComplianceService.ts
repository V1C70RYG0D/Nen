...
interface TransactionPattern {
  ...
  frequency: number; // transactions per hour
  isAnomalous: boolean;
}
...
class EnhancedComplianceService {
   ...
   */
  private async analyzeAmountRisk(walletAddress: string, amount: number): Promise<{score: number, flagged: boolean, reason: string}> {
    // Get user's transaction history
    const userStats = await this.dbService.cachedQuery(
      `user_stats:${walletAddress}`,
      async () => {
    ...
    const averageAmount = userStats.bets.reduce((sum: number, bet: any) => sum + bet.amount, 0) / userStats.bets.length;
    const maxAmount = Math.max(...userStats.bets.map((bet: any) => bet.amount));
   ...
   */
  private async analyzeFrequencyRisk(walletAddress: string): Promise<{score: number, flagged: boolean, reason: string}> {
   ...
   */
  private async analyzeBehaviorRisk(walletAddress: string): Promise<{score: number, flagged: boolean, reason: string}> {
    ...
    const roundNumbers = user.bets.filter((bet: any) => bet.amount % 1 === 0).length;
   ...
   */
  private async checkBlacklist(walletAddress: string): Promise<{score: number, flagged: boolean, reason: string}> {
   ...
   */
  private async performBlacklistCheck(walletAddress: string): Promise<boolean> {
   ...
   */
  private getRecommendedAction(riskScore: number): 'allow' | 'review' | 'block' {
  ...
  private async storeFraudDetectionResult(
    ...
    try {
      ...
      logger.error('Failed to store fraud detection result', {
        walletAddress,
        error: error instanceof Error ? error.message : String(error)
      });
    }
   ...
   */
  async checkKYCCompliance(walletAddress: string): Promise<KYCComplianceCheck> {
    try {
      const user = await this.dbService.getPrismaClient().user.findUnique({
        where: { wallet_address: walletAddress },
        select: {
      ...
      return {
        ...
        status: (user?.kyc_status as any) || 'pending',
  ...
  async getComplianceMetrics(): Promise<ComplianceMetrics> {
    try {
      const [totalUsers, compliantUsers, flaggedTransactions] = await Promise.all([
        ...
        this.dbService.getPrismaClient().$queryRaw<Array<{count: number}>>`
   ...
   */
  async analyzeTransactionPatterns(walletAddress: string): Promise<TransactionPattern> {
...
// Singleton instance
let complianceService: EnhancedComplianceService;

export const getEnhancedComplianceService = (): EnhancedComplianceService => {
