...
export const setupEnhancedGameSocket = (
  io: SocketIOServer,
  clusterService: WebSocketClusterService,
  magicBlockService: MagicBlockBOLTService,
  clusterManager: GeographicClusterManager,
  ...
  gameNamespace.on('connection', (socket) => {
    ...
    socket.on('create_game_session', async (data, callback) => {
      try {
        ...
        const magicBlockSessionId = await magicBlockService.createEnhancedSession(
          sessionId,
          socket.data.playerId, // Assuming player ID is set during auth
    ...
    socket.on('submit_move', async (data, callback) => {
      ...
      try {
        ...
        const moveResult = await magicBlockService.submitMoveEnhanced(
          sessionId,
          move,
          socket.data.playerId,
          crypto.randomBytes(32) // Generate secure anti-fraud token
        );
        ...
        const moveLatency = performance.now() - moveStartTime;
    ...
    socket.on('request_ai_move', async (data, callback) => {
      try {
        ...
        const moveResult = await magicBlockService.submitMoveEnhanced(
          sessionId,
          aiMove,
          socket.data.playerId, // AI moves through current player
          new Uint8Array(32)
        );
  ...
  function applyMoveToBoard(boardState: any, move: any) {
    ...
    try {
      ...
      if (newState.pieces) {
        const pieceIndex = newState.pieces.findIndex((p: any) =>
          p.x === move.fromX && p.y === move.fromY && p.level === move.fromLevel && p.player === move.player
        );
        ...
        if (pieceIndex >= 0) {
          ...
          // Check for captures
          const capturedIndex = newState.pieces.findIndex((p: any) =>
            p.x === move.toX && p.y === move.toY && p.level === move.toLevel && p.player !== move.player
          );
  ...
  function checkGameEndConditions(boardState: any) {
    // Check for Gungi win conditions
    try {
      const alivePieces = boardState.pieces?.filter((p: any) => p.alive) || [];

      // Check if King is captured (simplified win condition)
      const player1King = alivePieces.find((p: any) => p.player === 1 && p.type === 'King');
      const player2King = alivePieces.find((p: any) => p.player === 2 && p.type === 'King');
      ...
      if (boardState.moveCount >= moveLimit) {
        ...
        const player1Pieces = alivePieces.filter((p: any) => p.player === 1).length;
        const player2Pieces = alivePieces.filter((p: any) => p.player === 2).length;
  ...
  async function handleGameEnd(session: GameSession, result: any) {
  ...
  async function calculateAIMove(boardState: any, player: number, difficulty: string): Promise<any> {
    ...
    try {
      ...
      const aiPieces = boardState.pieces?.filter((piece: any) =>
        piece.player === player && piece.alive
      ) || [];
      ...
      switch (direction) {
        case 0: toY = Math.max(0, toY - moveDistance); break; // North
        case 1: toX = Math.min(8, toX + moveDistance); toY = Math.max(0, toY - moveDistance); break; // NE
        case 2: toX = Math.min(8, toX + moveDistance); break; // East
        case 3: toX = Math.min(8, toX + moveDistance); toY = Math.min(8, toY + moveDistance); break; // SE
        case 4: toY = Math.min(8, toY + moveDistance); break; // South
        case 5: toX = Math.max(0, toX - moveDistance); toY = Math.min(8, toY + moveDistance); break; // SW
        case 6: toX = Math.max(0, toX - moveDistance); break; // West
        case 7: toX = Math.max(0, toX - moveDistance); toY = Math.max(0, toY - moveDistance); break; // NW
      }
      ...
      return {
        ...
        toY,
        toLevel: 0,
        pieceType: selectedPiece.type || PieceType.Shinobi,
        player,
