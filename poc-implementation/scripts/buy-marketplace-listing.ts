...
async function main() {
  ...
  const listingPda = process.env.NEN_TEST_LISTING_PDA;
  const treasury = process.env.NEN_TREASURY_ADDRESS;
  const creator = process.env.NEN_CREATOR_ADDRESS; // optional override

  if (!programIdStr) throw new Error('NEN_MARKETPLACE_PROGRAM_ID not set');
  if (!mint) throw new Error('NEN_TEST_NFT_MINT not set');
  if (!listingPda) throw new Error('NEN_TEST_LISTING_PDA not set');
  if (!treasury) throw new Error('NEN_TREASURY_ADDRESS not set');
  ...
  const res = await client.buyListing({
    listingPda,
    mint,
    treasury,
    creator,
