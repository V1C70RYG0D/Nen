...
pub mod nen_marketplace {
    ...
    pub fn create_listing(ctx: Context<CreateListing>, price: u64, fee_bps: u16, listing_type: ListingType) -> Result<()> {
        require!(fee_bps <= 1000, MarketplaceError::InvalidFeeBps); // <= 10%
        ...
        listing.expires_at = clock.unix_timestamp + 30 * 24 * 60 * 60; // 30 days
        listing.status = ListingStatus::Active;
        listing.listing_type = listing_type;
        // Derive and store PDA bumps to avoid relying on ctx.bumps for this context
        let (escrow_pda, escrow_bump) = Pubkey::find_program_address(
            &[b"escrow_auth", ctx.accounts.mint.key().as_ref()],
            ctx.program_id,
        );
        require!(ctx.accounts.escrow_authority.key() == escrow_pda, MarketplaceError::InvalidEscrowAuth);
        listing.escrow_bump = escrow_bump;

        let (listing_pda, listing_bump) = Pubkey::find_program_address(
            &[b"listing", ctx.accounts.seller.key().as_ref(), ctx.accounts.mint.key().as_ref()],
            ctx.program_id,
        );
        require!(listing.key() == listing_pda, MarketplaceError::Unauthorized);
        listing.bump = listing_bump;

        // Transfer the NFT (1 token) from seller to escrow ATA
    ...
    pub fn cancel_listing(ctx: Context<CancelListing>) -> Result<()> {
        ...
        // Transfer NFT back to seller using escrow authority PDA as signer
...
pub struct EscrowAuthorityBump {
    pub bump: u8,
}
